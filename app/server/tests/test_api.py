...
from ..models import User, Project, Label, Document
...
class TestDocumentAPI(APITestCase):
    ...
    def test_get_docs_by_project_user(self):
        ...
        url = reverse('doc_list', args=[project.id])
        ...
        self.assertEqual(response.data['results'][0]['text'], doc.text)
    ...
    def test_get_doc_detail(self):
        ...
        doc = self.create_doc()
        project.documents.add(doc)
        url = reverse('doc_detail', args=[project.id, doc.id])
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIsInstance(response.data, dict)
        self.assertEqual(response.data['id'], doc.id)
        self.assertEqual(response.data['text'], doc.text)
